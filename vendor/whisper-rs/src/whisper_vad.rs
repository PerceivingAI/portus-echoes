...
use whisper_rs_sys::{
    whisper_vad_context, whisper_vad_context_params, whisper_vad_detect_speech,
    whisper_vad_detect_speech_no_reset, whisper_vad_free, whisper_vad_free_segments,
    whisper_vad_init_from_file_with_params, whisper_vad_n_probs, whisper_vad_params,
    whisper_vad_probs, whisper_vad_reset_state, whisper_vad_segments,
    whisper_vad_segments_from_probs, whisper_vad_segments_from_samples,
    whisper_vad_segments_get_segment_t0, whisper_vad_segments_get_segment_t1,
...
impl WhisperVadContext {
    ...
    /// Detect speech while preserving Silero's recurrent hidden/cell state.
    ///
    /// This is the streaming counterpart to [`Self::detect_speech`]. Call
    /// [`Self::reset_state`] once between independent utterances.
    pub fn detect_speech_no_reset(&mut self, samples: &[f32]) -> Result<(), WhisperError> {
        let (samples, len) = (samples.as_ptr(), samples.len() as c_int);
        let success = unsafe { whisper_vad_detect_speech_no_reset(self.ptr, samples, len) };
        if !success {
            Err(WhisperError::GenericError(-1))
        } else {
            Ok(())
    ...
    /// Reset Silero's recurrent hidden/cell state between independent utterances.
    pub fn reset_state(&mut self) {
        unsafe { whisper_vad_reset_state(self.ptr) }
    }
